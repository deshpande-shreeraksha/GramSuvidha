...
const SchemeApplications = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(t('Failed to fetch applications.'));
      }
    } catch (err) {
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      ...
      alert(t('Error connecting to backend.'));
    }
  };
    ...
    <div className="w-full space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-[#C4F8FF] tracking-tight flex items-center gap-3">
            ...
            {t("Admin Scheme's Portal")}
          </h1>
          <p className="text-[#C4F8FF]/70 mt-1 text-sm">
            {t('Manage scheme catalog listings and review submitted applications from citizens.')}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="font-bold text-[#C4F8FF] text-lg flex items-center gap-2">
          <Briefcase size={20} className="text-[#C4F8FF]" /> {t('Available Schemes Catalog')}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {SCHEMES_DB.map((scheme) => (
            ...
            >
              <div>
                <h3 className="font-extrabold text-base text-[#C4F8FF] mb-2">{t(scheme.name)}</h3>
                <p className="text-[#C4F8FF]/70 text-xs leading-relaxed mb-4">{t(scheme.desc)}</p>
                
                <div className="bg-[#0F4B70]/30 border border-[#C4F8FF]/20 p-3.5 rounded-xl space-y-1.5 text-[11px] text-[#C4F8FF]/80 mb-4">
                  <div>
                    <strong className="text-[#C4F8FF]">{t('Eligible Age Range:')} </strong> 
                    {scheme.minAge} - {scheme.maxAge} {t('years')}
                  </div>
                  <div>
                    <strong className="text-[#C4F8FF]">{t('Income Cap Limit:')} </strong> 
                    ₹{scheme.maxIncome.toLocaleString('en-IN')} {t('/ year')}
                  </div>
                </div>
              </div>
              ...
              >
                <Eye size={14} /> {t('View Scheme Setup')}
              </button>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4 pt-6 border-t border-slate-150">
        <h2 className="font-bold text-[#C4F8FF] text-lg flex items-center gap-2">
          <FileText size={20} className="text-[#C4F8FF]" /> {t('Submitted Applications Registry')}
          {applications.filter(a => a.status === 'Application Submitted').length > 0 && (
            <span className="bg-[#C4F8FF]/100 text-white text-[10px] px-2.5 py-0.5 rounded-full font-black ml-2">
              {applications.filter(a => a.status === 'Application Submitted').length} {t('Pending')}
            </span>
          )}
        </h2>
        {loading ? (
          <div className="p-12 text-center text-[#C4F8FF]/70">{t('Loading scheme applications...')}</div>
          ...
          <div className="text-center py-12 bg-[#0F4B70]/20 backdrop-blur-sm rounded-2xl border border-[#C4F8FF]/15">
            ...
            <p className="text-[#C4F8FF]/70 font-medium">{t('No scheme applications have been submitted yet.')}</p>
          </div>
        ) : (
              ...
              <div key={app._id} className="bg-[#0F4B70]/20 backdrop-blur-sm p-6 rounded-2xl border border-[#C4F8FF]/15 shadow-sm flex flex-col xl:flex-row gap-6 items-start xl:items-center justify-between">
                <div className="flex-1 space-y-3 w-full">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#C4F8FF]/20 pb-3">
                    <div>
                      ...
                      <span className="text-[10px] font-mono bg-[#0F4B70]/40 text-[#C4F8FF]/70 border border-[#C4F8FF]/15 px-2 py-0.5 rounded-md mt-1 inline-block">
                        {t('ID:')} {app.applicationId || 'SCH-N/A'}
                      </span>
                    </div>
                    ...
                    >
                      {app.status === 'Application Submitted' ? t('Pending Action') : t(app.status)}
                    </span>
                  </div>
                  
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                    <div>
                      <span className="text-[#C4F8FF]/60 block text-[9px] font-bold uppercase">{t('Applicant Name')}</span>
                    ...
                    <div>
                      <span className="text-[#C4F8FF]/60 block text-[9px] font-bold uppercase">{t('Income Declared')}</span>
                      <span className="font-bold text-[#C4F8FF]">₹{app.incomeDeclared ? app.incomeDeclared.toLocaleString('en-IN') : t('N/A')}</span>
                    </div>
                    <div>
                  ...
                  <div className="bg-[#0F4B70]/30 p-3 rounded-xl border border-[#C4F8FF]/20 flex flex-wrap gap-4 items-center text-xs">
                    <div>
                      <span className="text-[#C4F8FF]/60 font-medium mr-1.5">{t('ID Document:')}</span>
                      <span className="font-semibold text-[#C4F8FF]">{app.idNumber}</span>
                    </div>
                    {app.documentUrl && (
                      <div>
                        <span className="text-[#C4F8FF]/60 font-medium mr-1.5">{t('Uploaded File:')}</span>
                        <a href={app.documentUrl} target="_blank" rel="noreferrer" className="text-[#C4F8FF] hover:underline font-bold">{t('View Document')}</a>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap xl:flex-col gap-2.5 w-full xl:w-auto self-stretch justify-end border-t xl:border-t-0 xl:border-l border-[#C4F8FF]/20 pt-4 xl:pt-0 xl:pl-6">
                  {app.status === 'Application Submitted' ? (
                    <>
                      ...
                      >
                        <CheckCircle size={14} /> {t('Approve App')}
                      </button>
                      <button 
                    ...
                    >
                      <Clock size={14} /> {t('Set Back to Pending')}
                    </button>
                  )}
                </div>
              </div>
      ...
      {selectedScheme && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#0F4B70]/20 backdrop-blur-sm rounded-2xl shadow-xl max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="text-xl font-extrabold text-[#C4F8FF] mb-4">{t(selectedScheme.name)}</h2>
                ...
                lines.forEach((line, idx) => {
                  if (line.startsWith('•')) {
                    ...
                    if (trimmed.startsWith('**Overview**:')) {
                      ...
                      rendered.push(<p key={idx} className="font-bold text-[#C4F8FF] pt-2 border-t mb-2">{t('Benefits:')}</p>);
                    } else if (trimmed.startsWith('**Eligibility Criteria**:')) {
                      rendered.push(<p key={idx} className="font-bold text-[#C4F8FF] pt-2 border-t mb-2">{t('Eligibility Criteria:')}</p>);
                    } else if (trimmed.startsWith('**Documents Required**:')) {
                      rendered.push(<p key={idx} className="font-bold text-[#C4F8FF] pt-2 border-t mb-2">{t('Documents Required:')}</p>);
                    } else {
